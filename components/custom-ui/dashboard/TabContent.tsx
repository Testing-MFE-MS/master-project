...
export function TabContent({ activeTabId }: TabContentProps) {
  return (
      <>
        ...
        <div style={{ display: activeTabId === "workingPaper" ? "block" : "none" }}>
          <WorkingPaperComponent />
        </div>

        <div style={{ display: activeTabId === "user" ? "block" : "none" }}>
          <UserCreditComponent />
        </div>

        <div style={{ display: activeTabId === "reportsCCD" ? "block" : "none" }}>
          <ReportCCDProjectComponent />
        </div>

        {/* For other tabs, still use the switch approach */}
        {!["dashboard", "workingPaper", "user", "reportsCCD"].includes(activeTabId) && (
            <PlaceholderContent title={activeTabId} icon={Plus} />
        )}
      </>
  )
}